...
def main():
    st.set_page_config(
        page_title="Sigma Rule Browser & Converter",
        page_icon="🔍",
        layout="wide"
    )
    ...
    with col_reload:
        ...
        if st.button("🔄", help="Force reload conversion backend", key="reload_backend"):
            # Force reimport
            import importlib
            import sys
    ...
    if not SIGMA_REPO_PATH.exists():
        ...
        st.info("Please update SIGMA_REPO_PATH in the script to point to your Sigma repository.")
        return
    ...
    if search_term:
        filtered_rules = [
            ...
            if search_term.lower() in r.name.lower() or search_term.lower() in str(r.parent).lower()
        ]
    else:
    ...
    if rules_by_subdir:
        ...
        selected_subdir = st.sidebar.selectbox(
            ...
            format_func=lambda x: f"All ({len(filtered_rules)} rules)" if x == "All" else f"{x} ({subdir_counts.get(x, 0)} rules)",
            key="subdir_selector"
        )
        ...
        if browsable_rules:
            ...
            rule_options = {str(r.relative_to(SIGMA_REPO_PATH / selected_dir)): r for r in browsable_rules[:display_limit]}
            ...
            if len(browsable_rules) > display_limit:
                st.sidebar.caption(f"Showing first {display_limit} of {len(browsable_rules)} rules. Use search to narrow down.")

    st.sidebar.divider()
    st.sidebar.subheader("✅ Select for Conversion")

    # Use browsable_rules from subdirectory filter
    display_for_selection = browsable_rules[:50] if 'browsable_rules' in locals() else filtered_rules[:50]
        ...
        if "error" in rule_data:
            ...
            with st.container():
                ...
                with col1:
                    ...
                    st.markdown(f"<span style='color: #2063d6; font-size: 1.3rem; font-weight: 600;'>{rule_data.get('level', 'N/A').upper()}</span>", unsafe_allow_html=True)
                with col2:
                    st.markdown(f"**Status**")
                    st.markdown(f"<span style='color: #2063d6; font-size: 1.3rem; font-weight: 600;'>{rule_data.get('status', 'N/A')}</span>", unsafe_allow_html=True)
                with col3:
                    st.markdown(f"**Rule ID**")
                    rule_id = rule_data.get('id', 'N/A')
                    display_id = rule_id[:13] + "..." if rule_id and len(rule_id) > 16 else rule_id
                    st.markdown(f"<span style='color: #9db4c7; font-size: 1rem; font-family: monospace;'>{display_id}</span>", unsafe_allow_html=True)
                with col4:
                    st.markdown(f"**Author**")
                    author = rule_data.get('author', 'Unknown').split(',')[0]
                    st.markdown(f"<span style='color: #9db4c7; font-size: 1rem;'>{author}</span>", unsafe_allow_html=True)

                st.markdown(f"**Description:** {rule_data.get('description', 'No description available')}")

                # Show tags if present
                if 'tags' in rule_data and rule_data['tags']:
                    tags_display = " ".join([f"`{tag}`" for tag in rule_data['tags'][:10]])
                ...
                with col_right:
                    if 'detection' in rule_data:
                        ...
                        st.code(yaml.dump(rule_data['detection'], default_flow_style=False), language='yaml')
                ...
                with col_convert:
                    if st.button("🚀 Convert This Rule", use_container_width=True, type="primary"):
                        with st.spinner("Converting..."):
                            ...
                            if results and results[0].get('success'):
                                ...
                                score_value = rule_json.get('score_mapping', {}).get('default', 'N/A') if isinstance(rule_json.get('score_mapping'), dict) else rule_json.get('score', 'N/A')
                                info_col1.metric("Score", score_value)
                                info_col2.metric("Enabled", str(rule_json.get('enabled', 'N/A')))
                                info_col3.metric("Prototype", str(rule_json.get('is_prototype', rule_json.get('prototype', 'N/A'))))
                                info_col4.metric("Category", rule_json.get('category', 'N/A'))
                                ...
                                st.markdown("**Error:**")
                                st.code(results[0].get('error', 'Unknown error'), language='text')

                                # Show original YAML for debugging
                                if results[0].get('source_yaml'):
                                    with st.expander("📥 View Original Sigma Rule"):
                                        st.code(results[0]['source_yaml'], language='yaml')

                with col_add:
                    is_in_selection = st.session_state.preview_rule in st.session_state.selected_rules
                    if not is_in_selection:
                        if st.button("➕ Add to Batch", use_container_width=True):
                            st.session_state.selected_rules.append(st.session_state.preview_rule)
                            st.rerun()
                    else:
                        if st.button("➖ Remove", use_container_width=True):
                            st.session_state.selected_rules.remove(st.session_state.preview_rule)
                            st.rerun()
    ...
    tab_compare, tab1, tab2, tab3 = st.tabs(["⚡ Compare Mode", "📋 Selected Rules", "🔄 Convert", "📊 Results"])

    with tab_compare:
        st.markdown("### ⚡ Live Compare Mode")
        st.info("Browse rules and see instant conversion results. Perfect for rapid iteration!")

        # Confidence toggle
        col_toggle, col_info = st.columns([1, 3])
        with col_toggle:
            use_confidence = st.toggle("🎯 Confidence Gate", value=True, key="compare_confidence_toggle",
                                      help="Enable confidence-based field mapping validation")
        with col_info:
            if use_confidence:
                st.caption("✅ Confidence gate enabled - low-confidence field mappings will be flagged")
            else:
                st.caption("⚠️ Confidence gate disabled - all field mappings will be used")
        ...
        if st.session_state.preview_rule:
            ...
            with st.spinner("Converting..."):
                compare_results = convert_rules([st.session_state.preview_rule], enable_confidence=use_confidence)
            ...
            if compare_results and compare_results[0].get('success'):
                ...
                with col_input:
                    ...
                    if "error" not in rule_data:
                        ...
                        mcol3.metric("Category", rule_data.get('logsource', {}).get('category', 'N/A'))
                ...
                with col_output:
                    ...
                    with meta_col1:
                        ...
                        st.markdown(f"- **Content Type:** `{rule_json.get('content_type', 'N/A')}`")
                        st.markdown(f"- **Pattern Type:** `{rule_json.get('pattern_type', 'N/A')}`")
                        st.markdown(f"- **Stream:** `{rule_json.get('stream', 'N/A')}`")
                        st.markdown(f"- **Rule Source:** `{rule_json.get('rule_source', 'N/A')}`")

                    with meta_col2:
                        score_value = rule_json.get('score_mapping', {}).get('default', 'N/A')
                        st.markdown(f"- **Score:** `{score_value}` (type: `{rule_json.get('score_mapping', {}).get('type', 'N/A')}`)")
                        st.markdown(f"- **Enabled:** `{rule_json.get('enabled', 'N/A')}`")
                        st.markdown(f"- **Prototype:** `{rule_json.get('is_prototype', 'N/A')}`")
                        st.markdown(f"- **Category:** `{rule_json.get('category', 'N/A')}`")
                    ...
                    rule_key = st.session_state.preview_rule.stem if st.session_state.preview_rule else "none"
                    st.text_area(
                        label="expression",
                        value=expression,
                        height=150,
                    ...
                    if rule_json.get('entity_selectors'):
                        ...
                        for selector in rule_json['entity_selectors']:
                            st.markdown(f"- `{selector.get('entity_type')}` ← `{selector.get('expression')}`")

                    # Tags
                    if rule_json.get('tags'):
                        st.markdown("**Tags:**")
                        tags_str = ", ".join([f"`{tag}`" for tag in rule_json['tags'][:5]])
                ...
                if confidence_metadata and confidence_metadata.get('field_mappings'):
                    with st.expander("🎯 Field Mapping Confidence Analysis", expanded=False):
                        ...
                        for mapping in mappings:
                            ...
                            with col_score:
                                st.markdown(f"<span style='color: {conf_color}; font-weight: bold;'>{conf_emoji} {confidence:.2f}</span>",
                                          unsafe_allow_html=True)

                            with col_details:
                                with st.popover("Details"):
                                    st.markdown("**Factor Breakdown:**")
                                    st.markdown(f"- Semantic: {factors['semantic_similarity']:.2f}")
                                    st.markdown(f"- Data Preservation: {factors['data_preservation']:.2f}")
                                    st.markdown(f"- Type Compatibility: {factors['type_compatibility']:.2f}")
                                    st.markdown(f"- Field Specificity: {factors['field_specificity']:.2f}")
                        ...
                        st.divider()
                        st.caption(f"Category: {confidence_metadata.get('logsource_category', 'N/A')}")

                else:
                    st.caption("💡 Confidence metadata not available for this conversion")
    ...
    with tab1:
        if st.session_state.selected_rules:
            st.info(f"These {len(st.session_state.selected_rules)} rule(s) will be converted when you click the Convert button")
            ...
            for idx, rule_path in enumerate(st.session_state.selected_rules, 1):
                ...
                with col1:
                    ...
                    if "error" not in rule_data:
                        st.caption(f"{rule_data.get('title', 'Untitled')} - {rule_data.get('level', 'N/A')}")
            ...
            st.info("No rules selected for conversion. Use the sidebar to select rules, or use the preview panel above to browse and add rules.")
    ...
    with tab2:
        ...
        if st.session_state.selected_rules:
            ...
            if st.button("🚀 Convert to Sumo Logic CSE", type="primary", key="batch_convert"):
                try:
                    with st.spinner("Converting rules..."):
                        ...
                        if success_count > 0:
                            st.success(f"✅ Conversion complete! {success_count} successful, {error_count} errors")
                        else:
                            st.error(f"❌ All conversions failed. Check the Results tab for details.")
            ...
            for result in results:
                ...
                if result.get('success'):
                    with st.expander(f"✅ {source_file}"):
                        ...
                        col_info1.metric("Name", rule_json.get('name', 'N/A')[:20] + "..." if len(rule_json.get('name', '')) > 20 else rule_json.get('name', 'N/A'))
                        score_value = rule_json.get('score_mapping', {}).get('default', 'N/A') if isinstance(rule_json.get('score_mapping'), dict) else rule_json.get('score', 'N/A')
                        col_info2.metric("Score", score_value)
                        col_info3.metric("Enabled", rule_json.get('enabled', 'N/A'))
                        col_info4.metric("Prototype", rule_json.get('is_prototype', rule_json.get('prototype', 'N/A')))
                        col_info5.metric("Category", rule_json.get('category', 'N/A')[:15] + "..." if len(str(rule_json.get('category', ''))) > 15 else rule_json.get('category', 'N/A'))
                    ...
                    with st.expander(f"❌ {source_file}"):
                        ...
                        if 'source_yaml' in result and result['source_yaml']:
                            ...
                            st.warning("Original YAML not available (file read may have failed)")
        else:
            st.info("No conversion results yet. Go to the 'Convert' tab to convert selected rules.")
